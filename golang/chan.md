...
- 所有的操作都是围绕 buf，用来给协程进行IO数据
- recvq、sendq 用来处理并发的G，以及阻塞
- lock：保证并发安全
...
- 若等待接收队列 recvq 不为空，缓冲区中无数据或无缓冲区，将直接从 recvq 取出 G ，并把数据写入，最后把该 G 唤醒，结束发送过程。
...
closed ：先是打上一个标识，但管道里的值不做清空，释放所有 recvq 等待队列，并存入glist等待唤醒 ，再释放所有sendq，同样存入golist，等待唤醒 ，最后唤醒所有glist里的协程。
...
因为有 sendx  recvx 两个索引值，chan 的读写是可以并发的。但是，无法保证，同一时间可能有多个协和进行读操作，或者同一时间多个G进行写操作。
...
channel中传递指针会导致数据竞态问题（data race/ race conditions）
