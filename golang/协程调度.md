...
P：GOMAXPROCS，函数用来控制，开多少个P。
LRQ：执行G时，先从LRQ中找G，如果没有 再从 GRQ 中找。
GRQ：产生SYS_CALL，会进这里
#### runtime
...
大概的整体流程分析：
1. 尽可能的从队列中找一个待执行的 G
> local run queue 和  global  run queue
3. 执行GC
4. 把一些 sys_call 阻塞的 G 更新一下状态
5. 如果M被占用 ，解绑 P
...
会调用retake()函数，retake()函数会遍历所有的P，如果一个P处于执行状态， 且已经连续执行了较长时间，就会被抢占


- 由应用程序创建的计时器（timers）。sysmon查看应该在运行但仍在等待的计时器
