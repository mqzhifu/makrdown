...
辅助线程：持久化、异步删除、主从复制、过期处理、主从复制

3类线程：
BIO_CLOSE_FILE：关闭文件任务队列
BIO_AOF_FSYNC：AOF刷盘任务队列   bgsave   bgrewriteaof  
BIO_LAZY_FREE：异步删除
...
异步删除：打个删除标记，立刻返回，不阻塞。之后 ，lazyfree(BIO_LAZY_FREE) 线程 介入 
>unlink ：它会计算成本，大于64的走异步 ，简单类型的直接del ，复杂的 像  集合，走异步删除更好些。